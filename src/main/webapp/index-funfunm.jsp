                ...
                <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                    ...
                    <ul class="nav navbar-nav navbar-right">
                        ...
                        <li>
                            ...
                            <button id="man_btn" type="button" class="btn btn-primary btn-lg" data-toggle="modal"
                                    data-target="#myModal">
...
<script type="text/javascript">
        ...
        $
            .each(
                ...
                function (index, item) {
                    ...
                    var shaBtn = $("<button></button>")
                        .addClass(
                            "btn btn-info btn-sm sha-btn")
                        .append(
                            $("<span></span>")
                                .addClass(
                                    "glyphicon glyphicon-share"))
                        .append("分享");
                    shaBtn.attr("sha-funid", item.id);
                    var shaBtnTd = $("<td></td>").append(shaBtn);
                    $("<tr></tr>").append(f_idTd)
                        .append(nameTd).append(
                        dateTd).append(sizeTd)
                        .append(typeTd).append(f_nameTd).append(lookBtnTd).append(dowBtnTd).append(shaBtnTd)
    ...
    $(document).on(
        ...
        ".look-btn",
        function () {
            var fName = $(this).parent().parent().find("td:eq(1)")
                .text();
            $.ajax({
    ...
    $(document).on(
        ...
        function () {
            ...
            if (confirm("确定要删除" + fName + "吗？")) {
                $.ajax({
                    url: "${pagaContext.request.contextPath} file/todelete?id=" +
                        +$(this).attr("delete-funid") + "&jobid=" + jobId,
                    type: "PUT",
                    success: function (result) {
                        alert(result.msg);
                        showPage(currPage);
    ...
    $("#m_trash").click(function () {
        location.href = "funm-trash.jsp?" + "                           jobId=" + jobid + "&" + "                              password=" + password;
    });
</script>
</body>
</html>